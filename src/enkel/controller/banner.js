...
module.exports = class extends enkel.controller.base {
  ...
  async listAction () {
    ...
    try {
      let res = await this.EnkelBannerModel.findAll({
      ...
      if (res) {
        let _countAll = await this.EnkelBannerModel.count({
          where: _searchConditions
        });
        return this.json({
  ...
  async addAction () {
    ...
    try {
      ...
      let createdData = await this.EnkelBannerModel.create(_requestData);
      if (createdData) {
  ...
  async delAction () {
    ...
    try {
      let response = await this.EnkelBannerModel.destroy({
        where: {
          uuid: {
            [this.Op.like]: '%' + params.uuid
          }
